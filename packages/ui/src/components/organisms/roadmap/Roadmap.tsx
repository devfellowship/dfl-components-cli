...
export function Roadmap({ document, state, renderNode, onNodeClick, onAction, testIdPrefix = "roadmap", className = "", ariaLabel }: RoadmapProps) {
  ...
  return <section data-testid={testIdPrefix} aria-label={ariaLabel ?? document.title ?? "Roadmap"} className={`min-w-0 bg-[var(--c-roadmap-surface)] p-3 text-[var(--c-roadmap-neutral-fg)] ${className}`}>
    ...
    <div data-testid={`${testIdPrefix}-grid`} role="list" aria-label="Roadmap nodes" className="relative isolate grid min-w-0 grid-cols-3 items-center gap-x-3 gap-y-8 md:gap-x-8">
      ...
      {placements.map(({ node, row, column }) => {
        ...
        return <div key={node.id} role="listitem" data-testid={`${testIdPrefix}-node`} data-node-id={node.id} data-state={resolvedState}
          style={{ gridColumn: `${column.start} / span ${column.span}`, gridRow: `${row.start} / span 1` }} className="relative z-10 min-w-0 px-2 py-4">
